...
format:
  html: 
    embed-resources: true #if this is set as true then the html file will open the figures otherwise it cannot access the files if sent to someone else in html format
  pdf:
    pdf-engine: xelatex
    ...
    margin-bottom: 0.8in
    toc: true
    keep-tex: true #intermediate file to generate pdf. can edit this file in latex
    include-in-header:
      text: |
        \usepackage{lineno}
    include-before-body:
...
csl: elife.csl #change according to journal csl files can be found specifically for each journal. can also be downloaded from zotero
---
...
*italic* *italic* **bold** ^superscript^
